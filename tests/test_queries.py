...
async def test_query_as_valid(db: EZQL):
    ...
    users = await db.query_as(
        User, "SELECT id, name FROM users WHERE name = $1", "Nazar"
    )
...
async def test_query_as_empty(db: EZQL):
    users = await db.query_as(
        User, "SELECT id, name FROM users WHERE name = $1", "NonExistent"
    )
    assert users == []
...
async def test_query_insert_with_returning(db: EZQL):
    users = await db.query_as(
        User, "INSERT INTO users (name) VALUES ($1) RETURNING id, name", "Alice"
    )
...
async def test_query_update_with_returning(db: EZQL):
    users = await db.query_as(
        User, "INSERT INTO users (name) VALUES ($1) RETURNING id, name", "Charlie"
    )
    ...
    updated_users = await db.query_as(
        User,
...
async def test_query_update_without_returning(db: EZQL):
    users = await db.query_as(
        User, "INSERT INTO users (name) VALUES ($1) RETURNING id, name", "Dave"
    )
    ...
    returning = await db.query_as(
        User, "UPDATE users SET name = $1 WHERE id = $2", "Dave Updated", users[0].id
    )

    assert returning == []

    updated_users = await db.query_as(
        User, "SELECT id, name FROM users WHERE id = $1", users[0].id
    )
...
async def test_join_query(db: EZQL):
    user = await db.query_as(
        User, "INSERT INTO users (name) VALUES ($1) RETURNING id, name", "Nazar"
    )
    await db.execute(
...
async def test_transaction_commit(db: EZQL):
    ...
    users = await db.query_as(
        User, "SELECT id, name FROM users WHERE name = $1", "TransactionUser"
    )
    assert len(users) == 1
...
async def test_transaction_rollback(db: EZQL):
    ...
    users = await db.query_as(
        User, "SELECT id, name FROM users WHERE name = $1", "RollbackUser"
    )
    assert users == []
...
async def test_transaction_query_as(db: EZQL):
    async with db.transaction() as tx:
        ...
        users = await tx.query_as(
            User, "SELECT id, name FROM users WHERE name = $1", "TxQueryUser"
        )
